...
private:
    ...
    struct LonghandIteratorBase {
        void operator++() { ++index; }
        constexpr bool operator==(std::nullptr_t) const { return index >= serializer.length(); }
    ...
    static String serializedValue(Longhand);
...
String StyleProperties::getPropertyValue(CSSPropertyID propertyID) const
{
    if (auto value = getPropertyCSSValue(propertyID))
        return serializeLonghandValue(propertyID, *value);
...
inline String ShorthandSerializer::serializedValue(Longhand longhand)
{
...
{
    return serializedValue(longhand(index));
}
...
std::optional<Color> StyleProperties::propertyAsColor(CSSPropertyID property) const
{
    ...
    return primitiveColor.isRGBColor() ? primitiveColor.color() : CSSParser::parseColorWithoutContext(serializeLonghandValue(property, *colorValue));
}
...
String ShorthandSerializer::serializeFont() const
{
    ...
    return makeString(style, capsSeparator, caps, weightSeparator, weight, stretchSeparator, stretch, sizeSeparator,
        serializeLonghandValue(sizeIndex), lineHeightSeparator, lineHeight, ' ', serializeLonghandValue(familyIndex));
}
...
String ShorthandSerializer::serializeFontSynthesis() const
{
    ...
    bool weight = !isLonghandValueNone(longhandIndex(0, CSSPropertyFontSynthesisWeight));
    bool style = !isLonghandValueNone(longhandIndex(1, CSSPropertyFontSynthesisStyle));
    bool caps = !isLonghandValueNone(longhandIndex(2, CSSPropertyFontSynthesisSmallCaps));

    if (!weight && !style && !caps)
...
String ShorthandSerializer::serializeGrid() const
{
    ...
    return makeString("auto-flow", dense, " ", serializeLonghandValue(autoColumnsIndex), " / ", serializeLonghandValue(columnsIndex));
}
...
String ShorthandSerializer::serializeLonghandsOmittingInitialValues() const
{
    ...
    for (auto longhand : longhands()) {
        if (!isInitialValue(longhand))
            result.append(std::exchange(prefix, " "), serializedValue(longhand));
    }
...
String ShorthandSerializer::serializeCommonValue() const
{
    String result;
    for (auto longhand : longhands()) {
        String text = serializedValue(longhand);
        if (result.isNull())
            result = text;
        else if (result != text)
            return String();
...
String ShorthandSerializer::serializeBorderImage() const
{
    ...
    for (auto longhand : longhands()) {
        ...
        if (auto* width = dynamicDowncast<CSSBorderImageWidthValue>(longhand.value)) {
            ...
            valueText = serializedValue(longhand);
...
{
    auto sectionsTotalLength = sectionLength * 3;
    ASSERT(sectionsTotalLength <= length());

    if (!subsequentLonghandsHaveInitialValues(sectionsTotalLength))
        return String();

    // Serializable properties are widths, then styles, then colors.
    auto serializeSection = [&](unsigned sectionStartIndex) {
        String result;
        for (unsigned i = 0; i < sectionLength; ++i) {
            String text = serializeLonghandValue(sectionStartIndex + i);
            if (result.isNull())
                result = text;
            else if (result != text)
                return String();
    ...
    auto widthText = serializeSection(0);
    if (widthText.isNull())
        return String();
    auto styleText = serializeSection(sectionLength);
    if (styleText.isNull())
        return String();
    auto colorText = serializeSection(sectionLength + sectionLength);
    if (colorText.isNull())
        return String();
...
bool MutableStyleProperties::removeShorthandProperty(CSSPropertyID propertyID, String* returnText)
{
    ...
    if (!shorthand.length())
        return false;
    if (returnText)
        *returnText = getPropertyValue(propertyID);
...
bool MutableStyleProperties::removePropertyAtIndex(int index, String* returnText)
{
    ...
    if (returnText) {
        ...
        *returnText = serializeLonghandValue(property.id(), *property.value());
    }
...
StringBuilder StyleProperties::asTextInternal() const
{
    ...
    for (auto property : *this) {
        ...
        if (value.isNull())
            value = serializeLonghandValue(propertyID, *property.value());
...
{
    return makeString(cssName(), ": ", serializeLonghandValue(id(), *m_value), isImportant() ? " !important;" : ";");
}
