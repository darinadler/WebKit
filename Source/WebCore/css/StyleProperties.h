...
protected:
    ...
    explicit StyleProperties(CSSParserMode mode, Mutability mutability)
        : m_cssParserMode(mode)
        , m_isMutable(mutability != Immutable)
    { }

    StyleProperties(CSSParserMode mode, unsigned immutablePropertyCount)
        : m_cssParserMode(mode)
        , m_immutablePropertyCount(immutablePropertyCount)
    { }
